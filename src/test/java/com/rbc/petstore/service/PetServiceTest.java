...
public class PetServiceTest {
    ...
    public void setup() {
        ...
        wally = new Pet(null,"wally",InventoryStatus.PENDING);
    }
    ...
    public void test_createPet() {
        when(petRepository.save(wally)).thenReturn(wally);

        Pet result = petService.createPet(wally);

        Assert.assertNotNull(result);
        Assert.assertEquals(wally.getName(), result.getName());
    }
}
